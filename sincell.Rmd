...
This R/Bioconductor package (1) asseses cell-to-cell distance and (2) build a 
...
control metrics and that gene expression has been already normalized for 
library size, sequencing depth, or other sources of technical variability. 

#### Experimental filtering metrics

Single-cell RNA-Seq protocol may have imaged cells after capture, but prior to lysis. It is important to remove (1) libraries that did not come from a single cell, (2) libraries that were made from empty wells or (2) wells with excess cell debris. These are all problematic for sincell. This can all be stored in the `phenoData` table and filtered. 
...
(`expressionmatrix`). This holds the pre-processed expression values. 
...
dimension first and then can compute the cell-to-cell distance using 
